...
const ready = {
  ...
  async execute(client: Client) {
    ...
    const challenges = await messageFetchLimiter
      .key(challengeLogsChannelID)
      .schedule(() =>
        limiter.schedule(() => challengeLogs.messages.fetch({ limit: 30 }))
      )
      .catch(() => null)
      ...
      for (let message of challenges) {
        ...
        if (
          ...
          if (
            ...
            const challenger = await memberFetchLimiter
              .key(`${guild.id}_${challengerID}`)
              .schedule(() =>
                limiter.schedule(() => guild.members.fetch(challengerID))
              )
              .catch(() => null)
    ...
    const lcsRunsMessages = await messageFetchLimiter
      .key(lcsRunsChannel.id)
      .schedule(() =>
        limiter.schedule(() => lcsRunsChannel.messages.fetch({ limit: 30 }))
      )
      .catch(() => null)
    ...
    if (lcsRunsMessages)
      for (let message of lcsRunsMessages) {
        if (
          ...
          let logEmbedQuery: Collection<string, Message<true>> | null
          ...
          if (
            ...
            if (Date.now() - timestamp > 18000000 * 3) {
              logEmbedQuery = await messageFetchLimiter
                .key(lcsRunsChannel.id)
                .schedule(() =>
                  limiter.schedule(() =>
            ...
            if (logsChannel?.isSendable())
              await sendLimiter
                .key(logsChannel.id)
                .schedule(() =>
                  limiter.schedule(() => logsChannel.send({ embeds: [embed] }))
                )
                .catch(() => null)
          }
        }
